  ...
  try {
    ...
    if (!success)
      return {
        ...
        data: null,
        error: errorValidation.flatten().fieldErrors,
        code: 400,
      };

    const user = await get_user_login(result.identifier);
    if (!user || !(await compare_data(result.password, user.password)))
      return {
    ...
    return { message: "unexpected error", error: {}, data: null, code: 500 };
  }
}
